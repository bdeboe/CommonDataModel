/*
  InterSystems IRIS OMOP CDM Indices
  There are no unique indices created because it is assumed that the primary key constraints have been run prior to
  implementing indices.

  Notes on InterSystems IRIS SQL
    - A Primary Key already implies an index. Duplicate index definitions have been removed from the DDL below.
    - When using Columnar Storage for a table, further indices may not offer much added value and have been commented
      out in the DDL below. They may be restored if high selectivity / query patterns warrant so.
...
CREATE BITMAP INDEX idx_gender ON @cdmDatabaseSchema.person (gender_concept_id)
;
...
;
CREATE BITMAP INDEX idx_concept_vocabluary_id ON @cdmDatabaseSchema.concept (vocabulary_id ASC)
;
CREATE BITMAP INDEX idx_concept_domain_id ON @cdmDatabaseSchema.concept (domain_id ASC)
;
CREATE BITMAP INDEX idx_concept_class_id ON @cdmDatabaseSchema.concept (concept_class_id ASC)
;
...
;
CREATE BITMAP INDEX idx_concept_relationship_id_3 ON @cdmDatabaseSchema.concept_relationship (relationship_id ASC)
;
...
CREATE BITMAP INDEX idx_source_to_concept_map_1 ON @cdmDatabaseSchema.source_to_concept_map (source_vocabulary_id ASC)
;
CREATE BITMAP INDEX idx_source_to_concept_map_2 ON @cdmDatabaseSchema.source_to_concept_map (target_vocabulary_id ASC)
;
